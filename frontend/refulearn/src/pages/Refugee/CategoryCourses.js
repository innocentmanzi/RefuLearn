...
const CategoryCourses = () => {
  ...
  useEffect(() => {
    const initializeOfflineService = async () => {
      try {
        console.log('🔧 Initializing offline integration service for CategoryCourses...');
  ...
  useEffect(() => {
    ...
    //   console.log('🔄 Force refreshing completion data on mount for category:', category);
    //   try {
    ...
    //               
    //               console.log('🔄 Mount completion status for category:', {
    //                 category: category,
    ...
    console.log('🚫 CategoryCourses: Disabled forceRefreshCompletionData - keeping completedCourses empty');
    ...
    const fetchData = async () => {
      ...
      const fetchOfflineData = async () => {
        try {
          console.log('📱 Fetching offline data for category:', category);
          ...
          console.log('📱 Offline integration service data:', {
          ...
          if (courses.length === 0) {
            console.log('📱 No courses from offline service, trying localStorage...');
            const cachedCourses = localStorage.getItem(`refugee_category_${category}_courses`);
            if (cachedCourses) {
              courses = JSON.parse(cachedCourses);
              console.log('📱 Found courses in localStorage:', courses.length);
            }
          }
          
          // If no enrolled courses from offline service, try localStorage fallback
          if (enrolled.length === 0) {
            console.log('📱 No enrolled courses from offline service, trying localStorage...');
            const cachedEnrolled = localStorage.getItem('refugee_enrolled_cache');
            if (cachedEnrolled) {
              enrolled = JSON.parse(cachedEnrolled);
              console.log('📱 Found enrolled courses in localStorage:', enrolled.length);
            }
          }
          
          console.log('📱 Final offline data for category:', category, {
            courses: courses.length,
            enrolled: enrolled.length
          });
          ...
          try {
            ...
            console.log('📱 Fallback to localStorage only:', {
              courses: courses.length,
              enrolled: enrolled.length
            });
      ...
      try {
        ...
        if (isOnline && !token) {
          console.error('❌ No authentication token found for online mode');
  ...
  const isCompleted = (courseId) => {
    ...
    console.log(`🚫 FORCED: isCompleted(${courseId}) returning false`);
    return false;
  };
  ...
  if (error) {
    ...
    if (courses.length > 0) {
      console.log('📚 Showing cached courses despite error:', courses.length);
        ...
        {filteredCourses.length === 0 ? (
          ...
          <CourseGrid>
            {filteredCourses.map((course) => {
              ...
              console.log('🎨 Course card image data:', {
                courseId: course._id,
                ...
                finalImage: finalImageUrl
                  ...
                  <CourseCard key={course._id || Math.random()} onClick={() => handleViewCourse(course)}>
                    <CourseImage 
                      image={finalImageUrl}
                      onError={(e) => {
                        console.error('❌ Course card image failed to load:', finalImageUrl);
